...
function resizeImage(successCallback, errorCallback, file, targetWidth, targetHeight, encodingType) {
    ...
    /** 
        atualização 28/01 - Celso Henrique Leite
        loading PHOTOLIBRARY - BUG
		realese file.fileType add string type image
    ...
    **/
    ...
    if (file.fileType == ".jpg") {
        ...
        tempPhotoFileName = "camera_cordova_temp_return.jpeg";
    }
    /**/
...
/**
	Author: Celso Henrique Leite
	Realese - control Memory access MediaType Capture
	Data : 26/03/2016
	pt-BR: quando chamada varias vezes a camera trava e realizada a correção para apenas uma chamada controlando a memoria do aparelho
	version: windows phone 8.1
...
function takePictureFromCameraWP(successCallback, errorCallback, args) {
    ...
    var createCameraUI = function () {
        ...
        capturePreview.style.cssText = "position: fixed; left: 0; top: 0; width: 100%; height: 100%; z-index: 1010;";

        // Create capture button
        cameraCaptureButton = document.createElement("button");
        cameraCaptureButton.innerText = "Capturar";
        cameraCaptureButton.style.cssText = buttonStyle + "position: fixed; right: 0; bottom: 0; margin: 20px; z-index: 1020";

        // Create cancel button
        cameraCancelButton = document.createElement("button");
        cameraCancelButton.innerText = "Cancel";
        cameraCancelButton.style.cssText = buttonStyle + "position: fixed; left: 0; bottom: 0; margin: 20px; z-index: 1020";

        // camera background
        // Create capture button
        cameraBackground = document.createElement("div");
        cameraBackground.setAttribute("id", "cameraWP81");
        cameraBackground.style.cssText = "position: fixed; right: 0; bottom: 0; top:0 margin: 0; width: 1000px; height: 1000px; z-index: 1010;";
        /*
        buttonCameraBlock = document.createElement("div");
        buttonCameraBlock.setAttribute("id", "divCamera");
        buttonCameraBlock.style.cssText = "background-color: red; position: fixed; right: 0; bottom: 0; top:0 margin: 0; width: 30px; height: 30px; z-index: 1030;";
        */
    ...
    var startCameraPreview = function () {
        ...
        DeviceEnum.DeviceInformation.findAllAsync(DeviceEnum.DeviceClass.videoCapture).then(function (devices) {
            ...
            document.body.appendChild(cameraBackground);
            
            //document.body.appendChild(buttonCameraBlock);
    ...
    var destroyCameraPreview = function () {
        ...
        // Remove elements
        [capturePreview, cameraCaptureButton, cameraCancelButton, cameraBackground].forEach(function (elem) {
    ...
    var onCameraCaptureButtonClick = function () {
        ...
        if (this.getAttribute('clicked') !== null) {
            ...
            /*
            var node = document.getElementById('divCamera');
